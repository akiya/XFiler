...
| **￥** | ドライブのルートディレクトリへ移動 |
| **L** | ドライブ選択ダイアログを表示 |
| **Ctrl + L** | ディレクトリパスの直接入力モードへ移行 |
| **J** | 登録ディレクトリの選択ダイアログを表示 |
| **O** | 現在のペインのディレクトリを反対側のペインのディレクトリに合わせる |
...
| **End** | すべてのマークを解除 |
| **W** | 条件指定で一括マークするダイアログを表示 |

### ファイル操作

| キー操作 | 動作 |
|:---:|:---|
| **C** | 選択（マーク）したアイテムを反対側のペインへ**コピー** |
| **M** | 選択（マーク）したアイテムを反対側のペインへ**移動** |
| **D** | 選択（マーク）したアイテムを**削除** |
| **R** | カーソル位置のアイテムの名前を**変更**（リネーム） |
| **K** | 新しい**ディレクトリ（フォルダ）を作成** |
| **N** | 新しい**ファイルを作成** |
| **E** | カーソル位置のファイルを外部エディタで開く |
| **V** | カーソル位置のファイルを内蔵ビュアー（テキスト/バイナリ）で開く |
| **P** | 選択したアイテムを**圧縮**（Pack） |
| **U** | 選択したアーカイブファイルを**解凍**（Unpack） |
| **T** | 反対側のペインに**ショートカットを作成** |
| **Ctrl + C** | カーソル位置のアイテムの**パスをクリップボードにコピー** |
...
| **Q** | 現在のタブを閉じる |
| **Alt + ← / →** | 表示するタブを左右に切り替える |
| **S** | ソート順（名前、拡張子、サイズ、日付）の設定ダイアログを表示 |
| **F** | ファイル名のインクリメンタルサーチ |
| **F11** | 全画面表示の切り替え |
...
画面上部のタイトルバーを右クリックし、システムメニューから設定を行うことができます。
...
4.  **キー設定**
    ...
    *   特定の機能（ドライブ移動など）にはパラメータ（移動先パスなど）も設定できます。

### ディレクトリ設定(D)...
`J` キー (Jump) で呼び出せる「よく使うディレクトリ」を登録・編集するための専用ウィンドウを開きます。

*   1文字のショートカットキーを割り当てることで、`J` → `キー` の2ストロークで登録フォルダへ素早く移動できます。
*   一覧での並び順変更や、任意のキー (A-Z, 0-9等) への割り当てが可能です。
*   「新規項目」を追加して新しいディレクトリを登録できます。
